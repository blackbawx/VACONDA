...
def train(model, train_loader, val_loader, optimizer,
    ...
    while global_epoch < nepochs:
        ...
        for step, inputs in tqdm(enumerate(train_loader)):
            ...
            spk = Variable(inputs['spk']).cuda()
            mels_translated = Variable(inputs['mels_translated']).cuda()
            assert mels.shape[1] == linears.shape[1]

            mel_outputs, linear_outputs, vq_penalty, encoder_penalty, entropy = model(mels, mels_translated, spk)
            ...
            loss = reconstruction_loss * reconstruction_weight + vq_penalty + encoder_penalty * encoder_weight 
...
if __name__ == "__main__":
    ...
    feats_name = 'mspec'
    Mel_train = float_datasource(vox_dir + '/' + 'fnames.dur.train', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' + 'festival/falcon_' + feats_name)
    Mel_val = float_datasource(vox_dir + '/' + 'fnames.val', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' + 'festival/falcon_' + feats_name)

    feats_name = 'lspec'
    linear_train = float_datasource(vox_dir + '/' + 'fnames.dur.train', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' + 'festival/falcon_' + feats_name)
    ...
    # fnames_file, desc_file, feat_name, feats_dir, feats_dict
    feats_name = 'speaker'
    spk_train = categorical_datasource(vox_dir + '/' + 'fnames.dur.train', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' + 'festival/falcon_' + feats_name, spk_ids)
    spk_val = categorical_datasource(vox_dir + '/' + 'fnames.val', vox_dir + '/' + 'etc/falcon_feats.desc', feats_name, vox_dir + '/' + 'festival/falcon_' + feats_name, spk_ids)

    # Dataset and Dataloader setup
    trainset = VACONDADataset(linear_train, Mel_train, spk_train)
    train_loader = data_utils.DataLoader(
        trainset, batch_size=hparams.batch_size,
        num_workers=4, shuffle=True,
        collate_fn=collate_fn_vaconda, pin_memory=hparams.pin_memory)

    ## Ok champion, tell me where you are using this  
    valset = VACONDADataset(linear_val, Mel_val, spk_val)
