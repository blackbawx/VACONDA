...
class VACONDADataset(object):
    def __init__(self, X, Mel, spk):
        self.X = X
        self.Mel = Mel
        self.spk = spk
        
    def __getitem__(self, idx):
        return self.X[idx], self.Mel[idx], self.spk[idx]
    
    def __len__(self):
        return len(self.X)

def collate_fn_vaconda(batch): 
    #print(batch)
    seq_len = 32
    translation_length = 0
    max_offsets = [x[1].shape[0] - seq_len - translation_length*2 for x in batch] 
    mel_lengths = [x[1].shape[0] for x in batch] 
    #print("Max offsets and mel lengths: ", max_offsets, mel_lengths) 
 
    mel_offsets = [np.random.randint(translation_length*2, offset) for offset in max_offsets] 
  
    mel_ = [x[1] for x in batch] 
    linear_ = [x[0] for x in batch]
 
    mel = torch.FloatTensor([x[mel_offsets[i]:int(mel_offsets[i] + seq_len)] for i, x in enumerate(mel_)]) 
    linear = torch.FloatTensor([x[mel_offsets[i]:int(mel_offsets[i] + seq_len)] for i, x in enumerate(linear_)])

    #trs_offsets = [np.random.randint(0, translation_length*2) - translation_length for offset in max_offsets] 
    #mel_translated = torch.FloatTensor( [x [ mel_offsets[i]+trs_offsets[i] :  
    #                                      int(mel_offsets[i]+seq_len+trs_offsets[i])]  
    #                                      for i, x in enumerate(mel_)]) 
    spk = torch.LongTensor([x[2] for i, x in enumerate(batch)])

    batch = {}
    batch['mels'] = mel
    batch['mels_translated'] = mel
    batch['linears'] = linear
    batch['spk'] = spk
